...
export function TeacherCard({ teacher }: TeacherCardProps) {
  ...
  // قيم آمنة علشان ما يطيح runtime
  const totalStudents =
    typeof (teacher as any).totalStudents === "number"
      ? (teacher as any).totalStudents
      : 0;

  const averageRatingRaw =
    typeof (teacher as any).averageRating === "number"
      ? (teacher as any).averageRating
      : typeof (teacher as any).rating === "number"
      ? (teacher as any).rating
      : 0;
  ...
  const coursesCount = Array.isArray(teacher.courses)
